...
export async function setOrderStatus(formData: FormData) {
  ...
  if (!id || !status || !isAdmin()) return { ok: false, error: 'Unauthorized' };
  try {
...
export async function addOrderNote(formData: FormData) {
  ...
  if (!id || !note || !isAdmin()) return { ok: false, error: 'Unauthorized' };
  try {
...
export async function generateInvoiceAction(formData: FormData) {
  const id = String(formData.get('id') ?? '').trim();
  if (!id || !isAdmin()) return { ok: false, error: 'Unauthorized' };
  try {
...
export async function quickMarkAsPaid(formData: FormData) {
  ...
  try {
    ...
    return { ok: false, error: 'Failed to mark as paid' };
  }
}
