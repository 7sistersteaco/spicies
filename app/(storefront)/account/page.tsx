                    ...
                    <div key={order.id} className="lux-surface p-6 flex flex-col md:flex-row justify-between gap-6 hover:border-accent/40 transition-colors">
                      ...
                      <div className="flex flex-col md:items-end justify-between gap-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                          {order.type === 'order' ? (
                            <Link href={`/order-success?token=${order.public_token}`} className="w-full md:w-auto">
                              <Button variant="secondary" size="sm" className="w-full md:w-auto">Details</Button>
                            </Link>
                          ) : (
                            <a href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hi, checking status of my pre-order ${order.order_code}`)}`} target="_blank" rel="noopener noreferrer" className="w-full md:w-auto">
                              <Button variant="secondary" size="sm" className="w-full md:w-auto">Inquiry</Button>
                            </a>
                          )}
                        </div>
                      </div>
