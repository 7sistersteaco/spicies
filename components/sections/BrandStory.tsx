...
export default function BrandStory({ restroImageUrl }: { restroImageUrl?: string | null }) {
  const hasImage = isValidImageUrl(restroImageUrl);
      ...
      <Container className="grid gap-12 md:grid-cols-[0.9fr_1.1fr] md:items-center">
        <Reveal>
          <div className="relative h-[320px] w-full overflow-hidden rounded-3xl border border-white/5 bg-ink shadow-soft">
            <SafeImage
              src={hasImage ? restroImageUrl! : '/images/restro-story.svg'}
              fallback="/images/restro-story.svg"
              alt="7 Sisters Restro heritage"
              fill
              sizes="(max-width: 768px) 100vw, 45vw"
              className={hasImage ? 'object-cover object-center' : 'object-contain'}
            />
          </div>
        </Reveal>
        <Reveal delay={0.1}>
          <div className="space-y-6">
            ...
            <h2 className="text-3xl font-heading font-semibold md:text-4xl">What we serve daily, we now bring to you.</h2>
